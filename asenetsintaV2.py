...
def falmyynti3V():
    #vaakafal ylävasen
    falkuvat=["tyhja.png","v2falkuva.png", "v2falextremeV.png","v2falextremeV1.png", "v2falextremelyhyt.png", "v2falextremelyhyt2.png","v2fallyhyt"]
    falkuvamaara =len(falkuvat) - 1
    toka1=0
    #vf koordit
    ekay = 79
    tokay = 206
    ...
    for ii in range(0, 2):
       ...
       # for i in range (0,len(falkuvat-1)):
        for i in range (0,24):
            kierros=0
            print("kierros: ",i)
            #print("i%2= ",i%2)
            ...
            for y in range (0,falkuvamaara):




                img = cv2.imread(falkuvat[y])
                heightT, widthT, channels = img.shape

                #print("ok", widthT, "<=", VF[2]-VF[0])
                print("y-kierros",y)

                #
                pyautogui.moveTo(VF[0],VF[1])
                if widthT <= VF[2]-VF[0]:
                    etsittava = imagesearcharea(falkuvat[y],VF[0], VF[1], VF[2], VF[3], precision=0.5)


                    #kivaa = ImageGrab.grab(bbox=(VF[0],VF[1], VF[2], VF[3]))
                    #kivaa.show()
                    #jos listan ensimmainen kuva, eli tyhjan paikan kuva löytyy, siirrytään suoraan seuraavaan paikkaan
                    if y ==0 and etsittava != [-1,-1]:
                        VF[0] = 1516
                        VF[2] = 1900
                        break

                    elif etsittava!= [-1,-1]:
                        #print("löyty", falkuvat[y])
                        #kivaa = ImageGrab.grab(bbox=(VF[0],VF[1], VF[2], VF[3]))
                        #kivaa.show()
                ...
                kierros +=1
                # if kierros == len(falkuvat) and VF[2]>1890:
                #     VF[0] = 1260
                #     VF[2] = 1644

                #jos kierros ooli viimeinen, eikä löytynyt asetta, laitetaan loppupätkä kyseistä riviä etsittäväksi alueeksi
                if kierros == falkuvamaara:
                    VF[0]= 1390
                    VF[2] = 1900
                    break


        #lisää seuraavalle kierrokselle yhden ruudun koordinaatteja alaspäin
        a += 64
        a1 += 64
        VF[1]=78+a
        VF[3]=203+a
       ...
       #  vaakafal= imagesearcharea("falkuva.png",VF[0], VF[1], VF[2], VF[3])
       #  vaakafale= imagesearcharea("falextremeV.png",VF[0], VF[1], VF[2], VF[3])
       #  vaakafalel= imagesearcharea("lyhytfalV.png",VF[0], VF[1], VF[2], VF[3])
       #  vaakafalel2= imagesearcharea("lyhytfalV2.png",VF[0], VF[1], VF[2], VF[3])
       #  #jos fali löytyy, laitetaan koordinaatit ja aseen viemä tila talteen
       # # kivaa = ImageGrab.grab(bbox=(VF[0], VF[1], VF[2], VF[3]))
       # # kivaa = ImageGrab.grab(bbox=(1263, 79, 1641, 206))
       # # kivaa.show()
       #
       #  if vaakafal != [-1,-1]:
       #
       #      poistocoord += vaakafal
       #
       #      ymiinus += 126
       #      xmiinus += 379
       #      VF[0] += xmiinus
...
def falmyynti4P():
    ...
    alareuna = [0]
    #kuvat
    falkuvat=["v2falP.png","v2falePperus2.png","v2falePperus.png","v2falPL.png","v2falePLpidempi.png","v2falePL.png","v2falePL2.png"]
    #falkuvat listan pituus
    fkl = len(falkuvat)-1
    #scrollaus
    scroll = False
    #koko ala y
    ...
    for i in range(0, 4):
        kierros= 0
        lasku = 940-(VF[3]+etsinta[1])



        #TARVIS SAADA KOKO Y2 PLUSSAT, JOTTA TIETÄS KUINKA ALHAALLA OLLAAN JA VOIS SCROLLATA OIKEEN MÄÄRÄN ALAS JA JATKAA SEN
        #JÄLKEEN OIKEESTA  PAIAKSTA
        #print("nii iffiä?", lasku, "<260????")
        if scroll == True:
            scrollkierros = (VF[3]+etsinta[1])*0.013722126929674099485
            #print(int(scrollkierros))
            pyautogui.click(1872, 112)
            for i in range(0, int(scrollkierros)):
                pyautogui.scroll(-1)
            etsinta[0]=0
            etsinta[1]=0
        ...
        # if 940-(VF[3]+etsinta[1]) < 260:
        #
        #     print("JOOJOOJOO",)
        #     if VF[3]+etsinta[1] >765:
        #         if VF[3]+etsinta[1]==940:
        #             VF[1]=179
        #             etsinta[0]=0
        #             etsinta[1]=0
        #
        #         pyautogui.click(1559, 529)
        #         for i in range(1, 15):
        #             pyautogui.scroll(-1)
        ...
        for y in range (0,fkl+1):
            ...
            #
            pyautogui.moveTo(VF[0], VF[1])
            #print("pituusvertaus", heightT ,">", VF[3]-VF[1])
            print("koko ala y",VF[3]+etsinta[1])
            #jos kuvan koko on pienempi tai yhtäsuuri kuin scannattava alue
            print(falkuvat[y])
            if heightT <= (VF[3]+etsinta[1]) - (VF[1]+etsinta[0]):
                #print("debug", VF[3]+etsinta[1])
                etsittava = imagesearcharea(falkuvat[y], VF[0], VF[1]+etsinta[0], VF[2], VF[3]+etsinta[1], precision=0.5)

                kivaa = ImageGrab.grab(bbox=(VF[0], VF[1]+etsinta[0], VF[2], VF[3]+etsinta[1]))
                kivaa.show()
                # jos listan ensimmainen kuva, eli tyhjan paikan kuva löytyy, siirrytään suoraan seuraavaan paikkaan
                # if y == 0 and etsittava != [-1, -1]:
                #     VF[0] = 1516
                #     VF[2] = 1900
                #     break
                print("etsinta[1]", etsinta[1])
                if etsittava != [-1, -1]:
                    print("löyty", falkuvat[y])
                    print("y-askeli ",VF[3]+etsinta[1])
                    #lisätään ekaan y koordinaattiin kuvan korkeuden verran
                    etsinta[0]+= heightT

                    #jos y2 kuvan alueen etsinnän koordinaatti ei mene yli sivun alareunan, siihen lisätään suurimman aseen kuvan korkeuden verran pituutta
                    if VF[3]+etsinta[1]+378 <940:
                        etsinta[1] += 378
                        break
                    #
                    # elif 940 -etsinta[1] < 25:
                    #     print("etsinta plussa ELIF: ", 940 - etsinta[1], "<25")
                    #     alareuna = etsinta[1]
                    #     break
                        ...
                        #etsinta[1]= 481
                        break
                    #jos löytyy ase, lisätään sen allaolevan paikan y1-koordinaatiksi löydetyn aseen alareuna
            ...
            print("yala: ",yala)
                    ...
                    #jos ei löydy asetta, asetetaan sen alapuolella olevan etsintäkohdan y1-koordinaatiksi + kaksi ruutua
